...
client.on("messageCreate", async message => {
  ...
  switch(command) {
    ...
    case 'eldenring':
      ...
      if(erPhrase.includes("{word2}")){erWord2 = erW[Math.floor((Math.random() * (erW.length - 1)))];}
      erPhrase = erPhrase.replace("{word1}", erWord);
      erPhrase = erPhrase.replace("{word2}", erWord2);
      if (c < 0.2){
        let erWord = erW[Math.floor((Math.random() * (erW.length - 1)))]; console.log(erWord);
        let erPhrase2 = erP[Math.floor((Math.random() * (erP.length - 1)))];
        if(erPhrase2.includes("{word2}"))erWord2 = erW[Math.floor((Math.random() * (erW.length - 1)))];
        erPhrase2 = erPhrase2.replace("{word1}", erWord);
        erPhrase2 = erPhrase2.replace("{word2}", erWord2);
