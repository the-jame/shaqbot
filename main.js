...
var erP = [`{word} ahead`, `Likely {word}`, `If only I had a {word}`, `{word}, O {word}`,`Ahh, {word}`, `No {word} ahead`, `First off, {word}`, `Didn't expect {word}...`,`Behold, {word}!`,`{word}`,`{word} required ahead`,`Seek {word}`,`Visions of {word}...`,`Offer {word}`,`{word}!`,`Be wary of {word}`,`Still no {word}...`,`Could this be a {word}?`,`Praise the {word}`,`{word}?`,`Try {word}`, `Why is it always {word}?`, `Time for {word}`,`Let there be {word}`,`{word}...`];
...
client.on("messageCreate", async message => {
  ...
  switch(command) {
    ...
    case 'eldenring':
      ...
      erPhrase = erPhrase.replace("{word}", erWord);
      erPhrase = erPhrase.replace("{word}", erWord);
      if (c < 0.25){
        erWord = erW[Math.floor((Math.random() * (erW.length - 1)))];
        erPhrase2 = erPhrase2.replace("{word}", erWord);
        erPhrase2 = erPhrase2.replace("{word}", erWord);
